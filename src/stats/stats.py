...
def get_soo_stats(individuals, end):
    ...
    # #avgg=0;
    # temp = [i.fitness for i in individuals]
    # #print(temp)
    # avgg= np.nanmean(temp, axis=0)    
    avgg = 0
    if not trackers.best_ever or best > trackers.best_ever:
        # Save best individual in trackers.best_ever.
        trackers.best_ever = best
        #temp = [i.fitness for i in trackers.best_fitness_list]
        # avgg= np.nanmean(trackers.bes, axis=0)
        # trackers.avg_ever = avgg 


    # if not trackers.avg_ever or avgg < trackers.avg_ever:
    # # Save best individual in trackers.best_ever.
    #     #print(trackers.avg_ever,avgg)
    #     trackers.avg_ever = avgg
    ...
    if params['SAVE_PLOTS'] and not params['DEBUG']:
        if not end:
            ...
            avgg= np.nanmean(trackers.best_fitness_list, axis=0)
            trackers.avg_fitness_list.append(avgg)

        if params['VERBOSE'] or end:
            save_plot_from_data(trackers.best_fitness_list, "best_fitness")
            save_plot_from_data(trackers.avg_fitness_list, "avgs_fitness")
...
def get_moo_stats(individuals, end):
    ...
    #         print(ind.fitness)
    #     for ind in (trackers.best_ever):
    #         ind.fitness[1]=ind.fitness[1]*len(ind.genome)
    #     for ind in (trackers.best_ever):
    #         print(ind.fitness)
    ...
    if params['SAVE_PLOTS'] and not params['DEBUG']:
        ...
        all_arr2 = [[] for _ in range(params['FITNESS_FUNCTION'].num_obj)]

        # Generate array of fitness values.

        genome_array = np.array([len(ind.genome) for ind in trackers.best_ever])
        fitness_array=np.array([ind.fitness for ind in trackers.best_ever])
        #k = np.array(fitness_array)

        # print("start1")
        # print(fitness_array)
        # print(k)
        # for i in range(0,len(k)):
        #     fitness_array[i][1]=fitness_array[i][1]*
        # print('start2')
        # print(fitness_array)
        # print(k)
        second_array=np.array(fitness_array)
        fitness_array[:,1]=fitness_array[:,1]*genome_array
        ...
        # Add seconfpaired fitnesses to array for graphing.
        for fit in second_array:
            for o in range(params['FITNESS_FUNCTION'].num_obj):
                all_arr2[o].append(fit[o])

        if not end:
            trackers.second_pareto_list.append(all_arr2)
