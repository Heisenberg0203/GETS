...
class sma(base_ff):
    ...
    def evaluate(self, ind, **kwargs):
        ...
        if dist == "training":
            ...
            train = pd.Series(self.training_exp)

        elif dist == "test":
            # Set test datasets.
            train = pd.Series(self.training_exp)
            test= pd.Series(self.test_exp)
            #y = self.test_exp
        ...
        if params['OPTIMIZE_CONSTANTS']:
            ...
            if( dist == "test"):
                ...
                yhat=yhat.shift(lag)
                rows = yhat.isna().sum() 
                return np.sqrt(np.mean(np.square(test[rows:] - yhat[rows:])))

            return np.sqrt(np.mean(np.square(train[rows:] - yhat[rows:])))
