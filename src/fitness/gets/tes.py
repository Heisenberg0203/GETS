    ...
    def evaluate(self, ind, **kwargs):
        ...
        if params['OPTIMIZE_CONSTANTS']:
            ...
            if(dist=='test'):
                ...
                validseason=valido.copy()
                validforecast=valido.copy()
                
                for i in range(-step,len(valido)-step):
                    #print(i,m)
                    if(i<0):
                        #print(i+k,len(level)+i,len(level)+i-period+1)        
                        level[len(level)+i]=level[len(level)-1+i]+trend[len(level)+i-1]+alpha*(obsereved[len(level)+i]-level[len(level)+i-1]-trend[len(level)+i-1]-season[len(season)+i-period])
                        trend[len(level)+i]=trend[len(level)+i-1]+beta*(level[len(level)+i]-level[len(level)+i-1]-trend[len(level)+i-1])
                        season[len(level)+i]=season[len(level)+i-period]+gamma*(obsereved[len(level)+i]-level[len(level)+i]-season[len(level)+i-period])
                        validforecast[i+step]=level[len(level)+i]+step*trend[len(level)+i]+season[len(level)+i-period]
                    elif(i==0):
                        #print(i+k,0,len(level)+i-period+1)  
                        validlevel[0]=level[len(level)-1]+trend[len(level)-1]+alpha*(valido[0]-level[len(level)-1]-trend[len(level)-1]-season[len(season)-period])
                        validtrend[0]=trend[len(level)-1]+beta*(validlevel[0]-level[len(level)-1]-trend[len(level)-1])
                        validseason[0]=season[len(level)-period]+gamma*(valido[0]-validlevel[0]-season[len(level)-period])
                        validforecast[i+step]=validlevel[0]+step*validtrend[0]+season[len(level)+i-period]
                    else:
                        
                        if(i<=(period-1)):
                            #print(i+k,i,len(level)+i-period+1)
                            validlevel[i]=validlevel[i-1]+validtrend[i-1]+alpha*(valido[i]-validlevel[i-1]-validtrend[i-1]-season[len(season)+i-period])
                            validtrend[i]=validtrend[i-1]+beta*(validlevel[i]-validlevel[i-1]-validtrend[i-1])
                            validseason[i]=season[len(season)+i-period]+gamma*(valido[i]-validlevel[i]-season[len(season)+i-period])
                            validforecast[i+step]=validlevel[i]+step*validtrend[i] + season[len(level)+i-period]
                        else:
                            #print(i+k,i,i+k-period)
                            validlevel[i]=validlevel[i-1]+validtrend[i-1]+alpha*(valido[i]-validlevel[i-1]-validtrend[i-1]-validseason[i-period])
                            validtrend[i]=validtrend[i-1]+beta*(validlevel[i]-validlevel[i-1]-validtrend[i-1])
                            validseason[i]=validseason[i-period]+gamma*(valido[i]-validlevel[i]-validseason[i-period])
                            validforecast[i+step]=validlevel[i]+step*validtrend[i] + validseason[i+step-period]
            ...
            #a= np.append(a,np.sqrt(np.mean(np.square(x[m:] - forecast[m:]))))
            return np.sqrt(np.mean(np.square(x[period:] - forecast[period:])))
