...
from utilities.stats.trackers import second_pareto_list
...
def save_pareto_fitness_plot():
    ...
    ax1.set_xlabel('Regression_Error',fontsize=14)
    ax1.set_ylabel('Effective_Length', fontsize=14)
    ...
    # Plot data.
    for i, gen in enumerate(second_pareto_list):
        c = next(color)
        ax1.step(gen[0], gen[1], linestyle='--',
                 where=step_dir, color=c, lw=0.35, alpha=0.25)
        ax1.plot(gen[0], gen[1], 'o', color=c, ms=1)
    ...
    # Set labels with class names.
    ax1.set_xlabel('Regression_Error', fontsize=14)
    ax1.set_ylabel("EffectiveLength/ActualLength", fontsize=14)
    ...
    # Save plot and close.
    plt.savefig(path.join(params['FILE_PATH'], "fitness2.pdf"))
    plt.close()
...
def save_plot_from_data(data, name):
    ...
    # Initialise up figure instance.
    np.savetxt(path.join(params['FILE_PATH'], (name + 'array.txt')),data)
    fig = plt.figure()
    ax1 = fig.add_subplot(1, 1, 1)
